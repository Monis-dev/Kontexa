...
const AIService = {
  MODEL_NAME: "gemini-3.1-flash-lite-preview", // FIXED: 2.5 does not exist yet for this endpoint
  ...
  async callGemini(prompt, contextNotes) {
    ...
    // STRICT LIMIT: Only send the top 5 notes to the AI so we don't hit payload limits
    const safeNotes = contextNotes.slice(0, 5);
...
Return a JSON response with TWO things:
1. "answer" → answer to user question using ONLY the provided notes
2. "tags" → object mapping each exact note ID to an array of tags (lowercase, 2-4 words)

FORMAT:
{
  "answer": "your answer here",
  "tags": { "exact_note_id": ["tag1", "tag2"] }
}
...
${notesText || "No notes provided."}`;
    ...
    if (!raw) return { answer: "Empty AI response", tags: {} };

    raw = raw.replace(/```json|```/g, "").trim();
    const jsonMatch = raw.match(/\{[\s\S]*\}/); // Match object, not array
...
const systemPrompt = `You are an expert research assistant. Read the provided documentation/webpage content and extract the 3 to 5 most important concepts.
    ...
    Format:
    [
      { "title": "Short Heading", "content": "Detailed explanation (2-3 sentences).", "tags": ["tag1", "tag2", "tag3"] },
      { "title": "Another Heading", "content": "Another explanation.", "tags": ["tag1", "tag2"] }
    ]
    
    Tags rules: lowercase, 1-2 words, 2-4 tags per note, no symbols.
  ...
  async generateNotesFromQuestion(question) {
    ...
    const prompt = `You are a note generator. The user asked about a topic they have no saved notes on.
Generate 3 helpful, accurate notes about this topic. Return ONLY a valid JSON array.
FORMAT: [{ "title": "Concept Name", "content": "Clear explanation here.", "tags": ["tag1", "tag2"] }]
User Question: ${question}`;
    ...
    let raw = data?.candidates?.[0]?.content?.parts?.[0]?.text || "";
    raw = raw.replace(/```json|```/g, "").trim();
    const jsonMatch = raw.match(/\[[\s\S]*\]/); // match array
    try {
